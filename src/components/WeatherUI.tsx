import '../assets/weather_ui_design.css';
...
const WeatherUI = ({ weatherChoice }: WeatherUIProps) => {
  ...
  useEffect(() => {
    if (!weatherChoice.location) return;
  ...
  }, [weatherChoice.location])

  if (weatherChoice.option && weatherChoice.location) {
    return (
      <p>Showing Current Weather</p>
    )
  }
  else {
    return (
      <p>Search Weather UI</p>
    )
  }



  // const gainUserLocationPermission = () => {
  //   setIsDisabled(true);

  //   if (!("geolocation" in navigator)) {
  //     console.error("Sorry, your device doesn't support geolocation");
  //     setFade('fade-in');
  //     return
  //   }

  //   navigator.geolocation.getCurrentPosition(position => fetchCurrentWeatherData(position), 
  ...
  // const fetchCurrentWeatherData = async (position: GeolocationPosition) => {
  //   console.log('Retrieving location now');
  //   const { latitude, longitude } = position.coords;
  //   console.log(latitude, longitude);
  // }

  // const searchDifferentWeathers = () => {
  //   console.log('Search UI Appears');
  // }
  ...
  //       if (weatherData instanceof Weather) setWeather(weatherData);
  //     } 
  //     catch (e) {
  //       setError(true);
  ...
  //           <button type="button" className='bg-cyan-500 shadow-md shadow-cyan-500/90 rounded-full w-full my-2 py-2 text-md md:cursor-pointer disabled:cursor-not-allowed disabled:opacity-30 ' onClick={gainUserLocationPermission} disabled={`${isDisabled === 'disabled' ? 's' : 's'}`}>View Current Weather</button>
  //           <button type="button" className='bg-cyan-500 shadow-md shadow-cyan-500/90 rounded-full w-full py-2 text-md md:cursor-pointer disabled:cursor-not-allowed disabled:opacity-30' onClick={searchDifferentWeathers}>Search Different Weathers</button>
  //         </div>
  //       </div>
  //     </section>
  //   ) 
  ...
  const CanvasError = () => {
    ...
    return (
      <section className='fixed top-[0] left-[0] w-full h-full grid place-content-center'>
        ...
        <canvas className='error-design' width={300} height={300} ref={canvasRef} />
